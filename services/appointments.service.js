...
export class AppointmentService {
  ...
  async listSlots({ doctorId, dateISO }) {
    console.log("listSlots called with:", { doctorId, dateISO });
    if (!doctorId || !dateISO) {
      console.warn("Missing doctorId or dateISO, returning empty array", { doctorId, dateISO });
      return [];
    }
    const day = new Date(dateISO); // Use the input date as-is (UTC)
    try {
      const objectId = mongoose.Types.ObjectId.createFromHexString(doctorId); // Convert to ObjectId
      console.log("Querying Availability with:", { doctor: objectId, date: day, isBooked: false });
      const slots = await Availability.find({ doctor: objectId, date: day, isBooked: false })
        .sort({ startTime: 1 })
        .lean();
      console.log("Slots found:", slots);
      return slots;
    } catch (e) {
      console.error("Invalid doctorId format:", e.message);
      return [];
    }
  }
  ...
  async createAppointment({ patientId, doctorId, slotId }) {
    ...
    try {
      ...
      // 4) Return populated doc
      return Appointment.findById(appt._id)
        .populate({ path: "doctor", select: "firstName lastName email specialization licenseNumber" })
        .populate("availability")
        .lean();
    } catch (e) {
      await session.abortTransaction();
